...
//          webkit-playsinline="" playsinline="" x5-playsinline="" x-webkit-airplay="allow"
    ...
    methods: {
      ...
      showAudioStyle(player){
        ...
        player.el().className = 'audio-player';
      },
      onPlayerPlay(player) {
        if (this.type == 2) {
...
//        console.log('player current update state', playerCurrentState)
      ...
      playerReadied(player) {
        ...
        var hls = player.tech({IWillNotUseThisInPlugins: true,x5Playsinline:true}).hls
...
<style>
  ...
  .vjs-big-play-button::after{background-image: url(/statics/images/pubBack.png);background-repeat: no-repeat;background-size: 1rem 5rem;}
  ...
  /*播放按扭*/
  .video-player .video-js.vjs-custom-skin .vjs-big-play-button{position: absolute; top: 50%;left: 50%;margin:-.6rem 0 0 -.6rem!important;  width:1.2rem;height: 1.2rem!important;background:rgba(0,0,0,.4);border-radius:100%;border:none;}
